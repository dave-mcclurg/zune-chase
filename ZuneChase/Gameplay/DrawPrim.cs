...
// collaborative debugging tool from http://forums.xna.com/forums/t/7414.aspx
...
namespace ZuneChase.Gameplay
{
    ...
    public class BasicPrimitives
    {
        ...
        //////////////////////////////////////////////////////////////////////////
        public void CreateCircle(Vector2 _center, float _fRadius, int _nSides)
        {
            m_VectorList.Clear();
            ...
            {
                m_VectorList.Add(_center +
                    new Vector2(_fRadius * (float)Math.Cos((double)fTheta),
    ...
    public abstract class DrawPrim
    {
        public abstract void Submit(BasicPrimitives basicPrim);

        public class Line : DrawPrim
        {
            float x1, y1;
            float x2, y2;
            Color color;
            ...
            public override void Submit(BasicPrimitives basicPrim)
            {
                basicPrim.CreateLine(new Vector2(x1, y1), new Vector2(x2, y2));
                basicPrim.Colour = color;
            }
        }

        public class Circle : DrawPrim
        {
            Vector2 center;
            float radius;
            Color color;
            ...
            public override void Submit(BasicPrimitives basicPrim)
            {
                basicPrim.CreateCircle(center, radius, 64);
                basicPrim.Colour = color;
            }
        }
    }

    public class DrawPrims : GameplayComponent
    {
        BasicPrimitives basicPrim;
        List<DrawPrim> list = new List<DrawPrim>();

        public DrawPrims(GameplayScreen screen)
            : base(screen)
        {
            basicPrim = null;
        }
        ...
        {
            basicPrim = new BasicPrimitives(Screen.ScreenManager.GraphicsDevice);
        }

        public override void Draw()
        {
            foreach (DrawPrim drawPrim in list)
            {
                basicPrim.ClearVectors();
                drawPrim.Submit(basicPrim);
                basicPrim.RenderLinePrimitive(Screen.ScreenManager.SpriteBatch);
            }
            list.Clear();
        }
        ...
        {
            list.Add(new DrawPrim.Line(x1, y1, x2, y2, color));
        }

        public void AddCircle(Vector2 center, float radius, Color color)
        {
            list.Add(new DrawPrim.Circle(center, radius, color));
        }
    }
}